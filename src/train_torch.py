...
#     # Архитектура (можно оставить как есть, или вернуть более сложную)
#     model = MLPClassifier(
...
#     # ======================== Platt scaling ========================
#     # Загружаем лучшую модель для калибровки
...
def train_torch(cfg_path: str = "configs/default.yaml"):
    ...
    # === ИЗМЕНЕНИЕ 1: Уменьшенная емкость и усиленный dropout ===
    model = MLPClassifier(
        input_dim=X_train.shape[1],
        hidden_dim=16,      # Было 32
        dropout=0.5         # Было 0.4
    ).to(device)
    
    criterion = nn.BCEWithLogitsLoss()
    
    # === ИЗМЕНЕНИЕ 2: Усиленная L2-регуляризация ===
    optimizer = torch.optim.Adam(model.parameters(), lr=cfg["model"]["lr"], weight_decay=5e-2)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', patience=5, factor=0.5)
    ...
    for epoch in range(epochs):
        ...
        for X_b, y_b in train_loader:
            ...
            # === ИЗМЕНЕНИЕ 3: Более строгий клип градиентов ===
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=0.5)
            optimizer.step()
            epoch_loss += loss.item() * X_b.size(0)
    ...
    # === ИЗМЕНЕНИЕ 4: Сохранение конфигурации архитектуры модели ===
    model_config = {
        "input_dim": input_dim,
        "hidden_dim": 16,
        "dropout": 0.5
